        ...
        <CardHeader>
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
            <Avatar className="h-32 w-32">
              ...
              <AvatarFallback className="text-3xl">{getInitials(typedMember.full_name)}</AvatarFallback>
            </Avatar>
            <div className="flex-1 text-center sm:text-left">
              <CardTitle className="text-3xl mb-2">{typedMember.full_name}</CardTitle>
              <div className="flex flex-wrap gap-2 justify-center sm:justify-start">
                {typedMember.gender && (
                  <Badge variant="secondary">{typedMember.gender}</Badge>
                )}
        ...
        <CardContent className="space-y-8">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <div className="flex items-start gap-3 p-4 rounded-lg bg-gray-50 border border-gray-100">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-base font-semibold text-gray-900">{typedMember.phone}</p>
                </div>
              </div>
              {typedMember.email && (
                <div className="flex items-start gap-3 p-4 rounded-lg bg-gray-50 border border-gray-100">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-base font-semibold text-gray-900 break-all">{typedMember.email}</p>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <div className="p-4 rounded-lg bg-gray-50 border border-gray-100">
                ...
                <p className="text-base font-semibold text-gray-900">{formatDate(typedMember.date_of_birth, 'MMMM do, yyyy')}</p>
              </div>
              <div className="p-4 rounded-lg bg-gray-50 border border-gray-100">
                <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Birthday</p>
                <p className="text-base font-semibold text-gray-900">{formatBirthday(typedMember.date_of_birth)}</p>
              </div>
            </div>
          </div>
          ...
          {typedMember.address && (
            <div className="space-y-4">
              ...
              <div className="p-4 rounded-lg bg-gray-50 border border-gray-100">
                <p className="text-base font-semibold text-gray-900">{typedMember.address}</p>
              </div>
            </div>
          )}
